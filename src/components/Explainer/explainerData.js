...
export const overviews = {
  ...
  stems: <>
    <p>In the D3 visualization library that we used to build this project, there are a number of discrete and continuous scales to choose from. Below, you'll see several stems displayed useing the Quantize scale, which converts a range of continuous data (e.g. the active life of a repository) and maps that data to a list of discrete values.</p><p>We found that this scale was flexible in representing GitHub accounts with a wide variety of repository active life distributions without diluting the differences between repositories or making gardens with significant outliers unintelligible. That said, a different scale may have been more suitable if we had intended to highlight differences between the repositiories of multiple users rather than the internal differences in a single GitHub account.</p>
