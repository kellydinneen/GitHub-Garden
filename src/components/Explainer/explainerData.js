...
export const overviews = {
  ...
  stems: <>
    <p>In the D3 visualization library that we used to build this project, there are a number of discrete and continuous scales to choose from. Below, you'll see several stems displayed using a relative QUANTIZE SCALE, which converts a range of continuous data (e.g. the active life of a repository) to a set of discrete values.</p><p>We found that this scale was flexible in representing GitHub accounts with a wide variety of repository active life distributions without diluting the differences between repositories or making gardens with significant outliers unintelligible. That said, a different scale may have been more suitable if we had intended to highlight differences between the repositiories of multiple users rather than the internal differences in a single GitHub account.</p>
    <p>Interact with the inputs below to see how a different scale might work, or how different data could present itself. Note that multiple stems might change their size when you manipulate just one stem's corresponding value. This is because the relative scales are defined proportionally, so the scale itself changes whenever you change the data's maximum or minimum value.</p>
  </>,
  flowers: <>
    ...
    <p>Interact with these inputs to see how the sizes of the flowers change with a different scale or altered code volumes. Keep in mind that, as with the previous demonstration, multiple flowers might change their size when you manipulate just one flower's corresponding value. This is because the relative scales are defined proportionally, so the scale itself changes whenever you change the data's maximum or minimum value.</p>
  </>
}

export const scaleExplanations = {
  stems: {
    quantize: <p>The quantize scale breaks a range of possible values up into chunks and represents all the data that falls into a particular chunk with the same discrete value. E.g. a repo that is 1 day older than another may be represented by a flower that is no taller than the other, because they're both in the same "chunk". In this case, the quantize scale has 8 potential stem heights at intervals of 50 pixels. Because you have selected the relative version of the scale, it breaks up the data dynamically based on the minimum and maximum values. For instance, the tallest flower for a dataset whose maximum in 80 days will be the same height as the tallest flower for a dataset with a maximum of 1000 days, because the scale is determined proportionally</p>,
